...
int yearnum[4] = {365, 365, 365, 366};
char weekname[7][20] = {"Saturday", "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday"};
...
int getoldstyle(int d, int m, int y)
{
	if(oldleap(y))
	{
		return getstandard(d, m, y, lmonthnum) + (y-1) / 4;
	}
	else
	{
		return getstandard(d, m, y, monthnum) + (y-1) / 4;
	}
}
...
int getnewstyle(int d, int m, int y)
{
	if(newleap(y))
	{
		return getstandard(d, m, y, lmonthnum) + (y-1) / 4 - (y-1) / 100 + (y-1) / 400;
	}
	else
	{
		return getstandard(d, m, y, monthnum) + (y-1) / 4 - (y-1) / 100 + (y-1) / 400;
	}
}
