...
int main()
{
	...
	while(scanf("%s", buf) == 1)
	{
		...
		else if(!strcmp(buf, "UP"))
		{
			scanf("%d%d", &x, &y);
			releasearea = 0;
			if(x >= window[now].currx1 && x <= window[now].currx2 && y >= window[now].curry1 && y <= window[now].curry2)
			{
				if(y < window[now].curry1 + 25 && x < window[now].currx1 + 25)
				{
					releasearea = 1;
				}
				else if(y < window[now].curry1 + 25 && x > window[now].currx2 - 25)
				{
					releasearea = 2;
				}
				else if(y < window[now].curry1 + 25)
				{
					releasearea = 3;
				}
				else
			...
			if(releasearea == clickarea)
			{
				...
				{
					if(window[now].maximize)
					{
						window[now].maximize = 0;
						window[now].currx1 = window[now].origx1;
						window[now].currx2 = window[now].origx2;
						window[now].curry1 = window[now].origy1;
						window[now].curry2 = window[now].origy2;
					}
					else
					{
						window[now].maximize = 1;
						window[now].currx1 = 0;
						window[now].currx2 = 1023;
						window[now].curry1 = 0;
						window[now].curry2 = 1023;
					}
					printf("Resized window %d to %d, %d, %d, %d\n", window[now].id, window[now].currx1, window[now].curry1, window[now].currx2, window[now].curry2);
				}
			}
			if(clickarea == 3 && !window[now].maximize)
			{
				xshift = cx - x;
				yshift = cy - y;
				window[now].origx1 = window[now].currx1 -= xshift;
				window[now].origx2 = window[now].currx2 -= xshift;
				window[now].origy1 = window[now].curry1 -= yshift;
				window[now].origy2 = window[now].curry2 -= yshift;
