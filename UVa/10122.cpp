...
void minbsearch(int left, int right, int target)
{
	...
	for(; i<target; i++)
	{
		if(cross(x[target]-center, y[target], x[i]-center, y[i]) * cross(x[target]-center, y[target], x[n+1]-center, y[n+1]) > 0)
		{
			break;
		}
	}
...
void maxbsearch(int left, int right, int target)
{
	...
	for(; i>target; i--)
	{
		if(cross(x[target]-center, y[target], x[i]-center, y[i]) * cross(x[target]-center, y[target], x[0]-center, y[0]) > 0)
		{
			break;
		}
	}
