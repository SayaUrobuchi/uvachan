...
int main()
{
	int n, i, j, k, p, q, nd, dd, nx;
	int st, ed, stlan, edlan, l1, l2;
	data *temp, *now;
	while(scanf("%d", &n) == 1)
	{
		if(n == 0)
		...
		st = 0;
		ed = 1;
		memset(lptt, 0, sizeof(lptt));
		lhash[stlan][lptt[stlan]++] = st;
		lhash[edlan][lptt[edlan]++] = ed;
		for(i=0; i<n; i++)
		{
			scanf("%s%s%s", s1, s2, s3);
			p = i+2;
			let[p] = *s3;
			l1 = lan[p][0] = get_lan(s1);
			l2 = lan[p][1] = get_lan(s2);
			len[p] = strlen(s3);
			for(j=0, k=0; j<lptt[l1]||k<lptt[l2]; )
			{
				if(k >= lptt[l2] || (j < lptt[l1] && lhash[l1][j] <= lhash[l2][k]))
				{
					q = lhash[l1][j];
					if(lhash[l1][j] == lhash[l2][k])
					{
						k++;
					}
					j++;
