...
int main()
{
	int i, j, k, l, ns, step, over, meet, self, dir, res, idx, p, q, nl, cl;
	...
	for (i=0; i<3; i++)
	{
		...
		if (rep_len[i] - pat_len[i] > max_diff)
		{
			max_diff = rep_len[i] - pat_len[i];
		}
		if (rep_len[i] - pat_len[i] < min_diff)
		{
			min_diff = rep_len[i] - pat_len[i];
		}
	...
	for (i=0, j=2; i<j&&!over&&j<N; i++)
	{
		...
		{
			ns = qs[i] + 1;
			if (cl + (step-qs[i]) * max_diff < ql[1] || cl + (step-qs[i]) * min_diff > ql[1])
			{
				continue;
			}
		}
		...
		{
			ns = qs[i] - 1;
			if (cl - (qs[i]) * min_diff < ql[0] || cl - (qs[i]) * max_diff > ql[0])
			{
				continue;
			}
		}
