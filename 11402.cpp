...
int list[2008], start[1008], end[1008], num[2008], hash[1024005], repeat[1024005];
char type[1008], buf[1024005], str[1024005];
...
int main()
{
	...
	for(cas=0; ++cas<=count; )
	{
		...
		for(i=0, o=0; i<m; i++)
		{
			if(type[i] == 'F')
			{
				for(j=hash[start[i]], l=hash[end[i]]; j<l; num[j]=list[j]-list[j-1], j++);
			}
			else if(type[i] == 'E')
			{
			...
			{
				for(j=hash[start[i]], l=hash[end[i]]; j<l; num[j]=list[j]-list[j-1]-num[j], j++);
			}
			else
			{
